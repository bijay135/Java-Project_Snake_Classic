...
public class Engine {
    ...
    public void Move_mySnake(int newDirection,boolean food_eaten){
        if(!Game_Over){
            ...
            if(food_eaten){
                my_snake.Increase_length(); // IF the Snake ate new Food
            }
            my_snake.Move_snake();
    ...
    public void move_as_last(int direction){
        switch(direction){
            case 1:    
                my_snake.put_Head_postion(0,-step_to_move);
            break;//UP
    ...
    public int get_my_snake_length(){
        return my_snake.get_length();
    }
